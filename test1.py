...
def find_duplicate_images(folder_path, progress_bar, status_text, similarity_threshold=5):
    """
    Find duplicate or visually similar images using perceptual hashing and hamming distance.
    Returns: (duplicates_list, total_files_scanned)
    Where duplicates_list items are tuples: (duplicate_path, original_path, hamming_distance)
    """
    hashes = {}
    duplicates = []
            ...
            with Image.open(path) as img:
                ...
                for existing_hash, existing_path in hashes.items():
                    ...
                    if dist <= similarity_threshold:
                        duplicates.append((path, existing_path, dist))
                        found_match = True
                        break
...
def main():
    ...
    with st.sidebar:
        ...
        3. View duplicate or visually similar results
        """)
        st.info("Uses **perceptual hashing + hamming distance** to detect duplicates or near-duplicates.")
    ...
    if upload_choice == "📦 Upload ZIP File":
        ...
        if uploaded_zip:
            ...
            if extract_success:
                ...
                uploaded_files = [str(p) for p in Path(temp_dir).rglob("*.*")]
            else:
                st.stop()
    ...
    if uploaded_files:
        if st.button("🚀 Start Scanning", use_container_width=True):
            ...
            with st.spinner("🔎 Scanning for duplicates / near-duplicates..."):
                duplicates, total_files = find_duplicate_images(temp_dir, progress_bar, status_text, similarity_threshold=5)
            ...
            with col3:
                unique_count = total_files - len(duplicates)
                st.markdown(f"<div class='stats-box'><p class='stats-number'>{unique_count}</p><p class='stats-label'>Unique / Near-Unique Images</p></div>", unsafe_allow_html=True)

            st.markdown("---")

            if duplicates:
                st.header("🔁 Duplicate / Near-Duplicate Images Found")
                for idx, (dup_path, orig_path, dist) in enumerate(duplicates, 1):
                    with st.expander(f"Set #{idx} (Hamming dist = {dist})", expanded=True):
                        col1, col2 = st.columns(2)
                        with col1:
                            st.markdown("##### 📄 Original")
                            img1 = display_image(orig_path)
                            if img1:
                                st.image(img1, use_container_width=True)
                            st.text(f"📍 {os.path.basename(orig_path)}")
                            st.text(f"💾 {get_image_size(orig_path)}")
                        with col2:
                            st.markdown("##### 🔁 Duplicate / Similar")
                            img2 = display_image(dup_path)
                            if img2:
                                st.image(img2, use_container_width=True)
                            st.text(f"📍 {os.path.basename(dup_path)}")
                            st.text(f"💾 {get_image_size(dup_path)}")
            else:
                st.success("🎉 No duplicates or near-duplicates found! All images are unique.")
                st.balloons()
    else:
        st.markdown("""
        <div style='text-align: center; padding: 3rem;'>
            ...
            <p style='font-size: 1.2rem; color: #666; margin-top: 1rem;'>
                The tool will scan all images and find duplicates or near-duplicates.
            </p>
        </div>
        """, unsafe_allow_html=True)
